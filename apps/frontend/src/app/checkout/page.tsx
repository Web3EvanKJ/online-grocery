...
export default function CheckoutPage() {
  ...
  const { items, totalPrice, loading: cartLoading, fetchCart, clearCart } = useCartStore();
  ...
  const handleCheckout = async () => {
    ...
    try {
      setError(null);
      const orderData = { addressId: selectedAddressId, shippingMethodId: selectedShippingMethodId, voucherCode: voucherCode || undefined, notes: notes || undefined, paymentMethod };
      ...
      if (paymentMethod === 'payment_gateway') {
        const snapResult = await apiClient.initializeMidtransPayment(order.id); // or dynamic method
  ...
  if (cartLoading) return <div className="min-h-screen flex items-center justify-center"><LoadingSpinner size="lg"/></div>;
  if (items.length === 0) return <div className="min-h-screen flex items-center justify-center">Cart empty</div>;
  ...
  return (
    <div className="min-h-screen bg-blue-50 py-8">
      <div className="container mx-auto px-4 max-w-7xl">
        ...
        {error && <div className="mb-6 p-4 bg-blue-100 border border-blue-300 rounded-lg text-blue-800">{error}</div>}
